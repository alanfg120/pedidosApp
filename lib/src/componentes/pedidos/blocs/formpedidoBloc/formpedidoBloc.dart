...
class FormPedidosBloc extends Bloc<FormPedidoEvent,FormPedidoState>{
  ...
  if (event is AddCliente)
       yield* _mapAddClienteState(event);
  if (event is SearchEvent)
       yield* _mapSearchEventState(event);
  if (event is GetProducto)
       yield* _mapGetProductoState();
  if (event is GetCliente)
       yield* _mapetClienteState();
  if (event is UpdateProductoForm)
       yield* _mapUpdateProductoFormstate(event);
  if (event is UpdateClienteForm)
       yield* _mapUpdateClienteFormState(event);
  if (event is ResetProducto)
       yield* _mapResetProductostate();
  if (event is DeleteProductoForm)
       yield* _mapDeleteProductoFormstate(event);
  }
  ...
  if(event.tipo == 'productos'){
      final listaSugerida = state.productos.where((p)=> p.productoNombre
                                                      .toLowerCase()
  ...
  if(event.tipo == 'clientes'){
    final listaSugerida = state.clientes.where((c)=> c.nombre
                                                      .toLowerCase()
                                                      .startsWith(event.query.toLowerCase()))
                                                      .toList();
      yield state.copyWith(
  ...
  Stream<FormPedidoState> _mapGetProductoState() async* {
   ...
   yield state.copyWith(pedido:state.pedido,query:'',productos: initialProductos,clientes: state.clientes); 
  ...
  Stream<FormPedidoState> _mapUpdateProductoFormstate(UpdateProductoForm event) async* {
     ...
     yield state.copyWith(pedido:state.pedido,query:'',productos:state.productos,clientes: state.clientes); 
 ...
 Stream<FormPedidoState> _mapDeleteProductoFormstate(DeleteProductoForm event) async* {
    final indexProducto = state.productos.indexOf(state.pedido.productos[event.index]);
 ...
 Stream<FormPedidoState> _mapetClienteState() async* {
   ...
   yield state.copyWith(pedido:state.pedido,query:'',clientes: initialClientes,productos: state.productos); 
  ...
  Stream<FormPedidoState> _mapUpdateClienteFormState(UpdateClienteForm event) async* {
     bool exist = false;
